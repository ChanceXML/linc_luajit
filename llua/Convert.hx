...
public static function toLua(l:State, val:Any):Bool {
	...
	if (Std.isOfType(val, Bool)) {
		Lua.pushboolean(l, cast(val, Bool));
		return true;
	}
	...
	if (Std.isOfType(val, haxe.ds.StringMap) || Std.isOfType(val, haxe.ds.ObjectMap)) {
		mapToLua(l, val);
		return true;
	}

	if (Type.typeof(val) == Type.ValueType.TObject) {
		anonToLua(l, val);
		return true;
	}
